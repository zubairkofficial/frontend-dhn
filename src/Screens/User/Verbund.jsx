...
const Verbund = () => {
  ...
  const handleDownload = async () => {
    ...
    const produktnameCounts = allVerbundData.reduce((acc, fileData) => {
      const produktname = getVerbundProduktname(fileData.data);
      if (produktname) {
    ...
    allVerbundData.forEach((fileData) => {
      const rowValues = getVerbundRowValues(fileData.data);
      const sectionMissingCount = parseInt(
        rowValues[VERBUND_SECTION_MISSING_INDEX] || 0,
        10
      );
