...
// Surfachem column order (exact as per sample output - matches API response keys)
...
const Surfachem = () => {
  ...
  const [allSurfachemData, setAllSurfachemData] = useState([]);
  const [processedCount, setProcessedCount] = useState(0);
  const fileInputRef = useRef(null);
  const [refreshSurfachemData, setRefreshSurfachemData] = useState(false);

  const checkUsageCount = async () => {
    try {
      const response = await axios.get(
        `${Helpers.apiUrl}check-usage-count/Surfachem`,
        Helpers.authHeaders
      );
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        const response = await axios.post(
          `${Helpers.apiUrl}surfachem-data-process`,
          formData,
  ...
  const handleDownload = async () => {
    ...
    worksheet.columns = SURFACHEM_HEADERS.map(() => ({ width: 25 }));

    const buffer = await workbook.xlsx.writeBuffer();
    const blob = new Blob([buffer], {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    saveAs(blob, "SDB2Excel_Surfachem_Files.xlsx");
    setRefreshSurfachemData((prev) => !prev);
    setSelectedFiles([]);
    setFileStatuses({});
    setAllSurfachemData([]);
    ...
    <div className="w-full bg-white py-5 mx-auto">
        ...
        {selectedFiles.length > 0 && (
          <div className="w-full bg-gray-200 rounded-full h-4 mb-4 relative">
            ...
            <span className="absolute inset-0 flex justify-center items-center text-sm font-medium">
              {processedCount}/{selectedFiles.length} SDB verarbeitet
            </span>
          </div>
        )}
      ...
      <div className="flex justify-end gap-1 mt-8 px-10">
        <button
          ...
          className="flex justify-end text-white py-3 px-6 font-bold bg-success-300 hover:bg-success-400 transition-all rounded-lg"
      ...
      <div className="mt-10">
        <GetSurfachemData refresh={refreshSurfachemData} />
      </div>
    </div>
  );
};

export default Surfachem;
