...
import {
  VERBUND_HEADERS,
  getVerbundRowValues,
  getVerbundProduktname,
} from "../../Config/verbundColumns";
...
const GetVerbundData = ({ refresh }) => {
  ...
  const handleDownloadFile = async (fileName, fileData) => {
    ...
    const dataRow = worksheet.addRow(getVerbundRowValues(fileData));
  ...
  const handleDownloadAll = async () => {
    ...
    const produktnameCounts = filteredData.reduce((acc, item) => {
      const produktname = getVerbundProduktname(item.data);
      if (produktname) {
    ...
    filteredData.forEach((file) => {
      const newRow = worksheet.addRow(getVerbundRowValues(file.data));
      newRow.eachCell((cell) => {
        cell.alignment = { vertical: "middle", wrapText: true };
      });
      const produktname = getVerbundProduktname(file.data);
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p>
                    Product Name:{" "}
                    {truncateText(getVerbundProduktname(item.data))}
                  </p>
                  {item.created_at && (
