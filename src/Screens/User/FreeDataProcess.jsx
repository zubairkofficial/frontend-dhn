...
const FreeDataProcess = () => {
  ...
  const checkUsageCount = async () => {
    try {
      const response = await axios.get(
        `${Helpers.apiUrl}check-usage-count/FreeDataProcess`,
        Helpers.authHeaders
      );
  ...
  useEffect(() => {
    setHeaderData({
      title: Helpers.getTranslationValue("Kostenloser Datenprozess"),
      desc: "Dieses Tool ist für kostenlose Benutzer unter dem Cretschmar-Administrator verfügbar",
    });
    checkUsageCount();
  }, [setHeaderData]);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await axios.post(
        `${Helpers.apiUrl}send-processed-file`,
        formData,
        Helpers.authFileHeaders
      );
  ...
  return (
    <div className="w-full bg-white py-5 mx-auto">
      <h2 className="text-center text-2xl font-semibold mb-8">
        {Helpers.getTranslationValue("Kostenloser Datenprozess")}
      </h2>
